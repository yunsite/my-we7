...
function UEditor(selector) {
	...
	var option = {
		toolbars:[
			['fullscreen', 'source', '|', 'removeformat', 'bold', 'italic', 'underline', '|', 'forecolor', 'backcolor', 'fontsize', 'insertorderedlist', 'insertunorderedlist', '|', 'link', 'insertimage', 'insertvideo', 'music', 'map']
		],
		savePath : ['images'],
	...
	if (typeof UE == 'undefined') {
		$.getScript('./resource/script/ueditor/ueditor.config.js', function(){
			$.getScript('./resource/script/ueditor/ueditor.all.min.js',function(){
				$.getScript('./resource/script/ueditor/lang/zh-cn/zh-cn.js',function(){initUEditor()});
			});
		});
	} else {
		initUEditor();
	...
	function initUEditor() {
		$(selector).each(function(){
			if ($(this).get(0).tagName.toLowerCase() == 'textarea') {
				$(this).css("margin","0");
				if(!$(this).get(0).isload){
					$(this).get(0).isload = true;
					var editor = UE.ui.Editor(option);
					editor.render(this);
					//editor.setContent(selector.value);
				}
			}
		});
