...
$dos = array('installed', 'check', 'prepared', 'install', 'upgrade', 'uninstall', 'designer', 'permission', 'convert');
		...
		if($foo == 'upgrade') {
			...
			if (!empty($modules)) {
				foreach ($modules as $mid => $module) {
					...
					if(is_array($manifest) && $manifest['application']['version']) {
						$mods[$module['name']] = array('from' => 'local', 'version' => $manifest['application']['version']);
					}
				}
			}
			$ret = cloud_m_query();
...
if($do == 'permission') {
	...
	if(empty($module)) {
		message('你访问的模块不存在. 或许你愿意去微擎云服务平台看看. ', 'http://addons.we7.cc/search.php?q=' . $id);
	}
...
if($do == 'upgrade') {
	...
	if(pdo_update('modules', $module, array('name' => $id)) !== false) {
		cache_build_modules();
